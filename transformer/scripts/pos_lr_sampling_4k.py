...
plt.errorbar(ls_list, hyp_dict['4k'], se_dict['4k'], marker='o', label='sinusoid', color='blue', capsize=4)
plt.errorbar(ls_list, hyp_dict['pos_learned_4k'], se_dict['pos_learned_4k'], marker='o', label='learned', color='orange', capsize=4)
plt.errorbar(ls_list, hyp_dict['no_pos_4k'], se_dict['no_pos_4k'], marker='o', label='none', color='green', capsize=4)
plt.errorbar(ls_list, hyp_dict['rel_pos_4k'], se_dict['rel_pos_4k'], marker='o', label='relative', color='red', capsize=4)
plt.hlines(ref_dict['4k'][0], ls_list[0], ls_list[-1], label='test', color='grey', linestyle='dashed')
...
plt.grid()
#plt.title('Sampling mean sentence length for different positional embeddings with 4k bpe dictionary size (with SE)')
plt.ylabel('length')
plt.xlabel('label smoothing $\\lambda$')
plt.legend(bbox_to_anchor=(1.05, 1),
                         loc='upper left', borderaxespad=0.)
plt.savefig('img/pos_sampling_4k.pdf', dpi=400)
plt.clf()

plt.figure(figsize=(3.9*1.2,1.7*1.2))
plt.errorbar(ls_list, hyp_dict['4k'], std_dict['4k'], marker='o', label='sinusoid', color='blue', capsize=4)
plt.errorbar(ls_list, hyp_dict['pos_learned_4k'], std_dict['pos_learned_4k'], marker='o', label='learned', color='orange', capsize=4)
plt.errorbar(ls_list, hyp_dict['no_pos_4k'], std_dict['no_pos_4k'], marker='o', label='none', color='green', capsize=4)
plt.errorbar(ls_list, hyp_dict['rel_pos_4k'], std_dict['rel_pos_4k'], marker='o', label='relative', color='red', capsize=4)
plt.hlines(ref_dict['4k'][0], ls_list[0], ls_list[-1], label='test' , color='grey', linestyle='dashed')
plt.hlines(training_avg, ls_list[0], ls_list[-1], label='train', color='grey', linestyle='dotted')

plt.grid()
#plt.title('Sampling mean sentence length for different positional embeddings with 4k bpe dictionary size (with SD)')
plt.ylabel('length')
plt.xlabel('label smoothing $\\lambda$')
plt.legend(bbox_to_anchor=(1.05, 1),
                         loc='upper left', borderaxespad=0.)
plt.savefig('img/pos_sampling_4k_std.pdf', dpi=400)
plt.clf()
