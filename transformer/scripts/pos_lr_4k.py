...
plt.plot(ls_list, hyp_dict['4k'], '-o', label='sinusoid (4k)', color='blue')
plt.hlines(ref_dict['4k'][0], ls_list[0], ls_list[-1], color='grey', label='reference', linestyle='dashed')
plt.plot(ls_list, hyp_dict['pos_learned_4k'], '-o', label='learned', color='orange')
plt.plot(ls_list, hyp_dict['no_pos_4k'], '-o', label='no_pos_4k', color='green')
plt.plot(ls_list, hyp_dict['rel_pos_4k'], '-o', label='rel_pos_4k', color='red')
plt.grid()
plt.title('Beam search average sentence length for different positional embeddings')
plt.ylabel('length')
plt.xlabel('label smoothing alpha')
plt.legend()
plt.savefig('img/pos_length_4k.png', dpi=400)
plt.clf()

plt.figure(figsize=(12,7))
plt.plot(ls_list, bleu_dict['4k'], '-o', label='sinusoidal (4k)', color='blue')
plt.plot(ls_list, bleu_dict['pos_learned_4k'], '-o', label='learned', color='orange')
plt.plot(ls_list, bleu_dict['no_pos_4k'], '-o', label='no_pos_4k', color='green')
plt.plot(ls_list, bleu_dict['rel_pos_4k'], '-o', label='rel_pos_4k', color='red')
plt.grid()
plt.title('Beam search BLEU for different positional embeddings')
plt.ylabel('BLEU')
plt.xlabel('label smoothing alpha')
plt.legend()
plt.savefig('img/pos_bleu_4k.png', dpi=400)
